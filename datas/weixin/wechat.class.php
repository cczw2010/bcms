...
class Wechat
{
	...
	const USER_LISTINFO_URL='/user/info/batchget?';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		return @file_put_contents($_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.$cachename.'.caches',$str);
	}
	/**
	 * 获取缓存，按需重载
	 * @param string $cachename
	...
	protected function getCache($cachename){
		if($info = @file_get_contents($_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.$cachename.'.caches')){
			if($json = json_decode($info)){
				$time = time();
				if ($time-$json->stime < $json->expired) {
					return $json->token;
				}
			}
		}
	...
	protected function removeCache($cachename){
		return @unlink($_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.$cachename.'.caches');
	}
	/**
	 * 获取access_token
	 ...
	 *        [{
	 *        	"openid": "xxx", 
	 *        	"lang": "zh-CN" //可以省略  默认zh-CN
	 *        	},...
	 *        ]
	 * @return array {subscribe,openid,nickname,sex,city,province,country,language,headimgurl,subscribe_time,[unionid]}
	 * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
	 */
	public function getUserInfos($openids){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$params = array('user_list'=>$openids);
		$result = $this->http_post(self::API_URL_PREFIX.self::USER_LISTINFO_URL.'access_token='.$this->access_token,self::json_encode($params));
		if ($result)
		{
			$json = json_decode($result,true);
			if (isset($json['errcode'])) {
