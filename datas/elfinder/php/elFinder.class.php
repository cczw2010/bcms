...
class elFinder {
	...
	/**
	 * 是否重命名上传的文件  by awen @2014-6-13
	 * @var boolean
	 */
	protected $uploadRename = false;
	...
	public function __construct($opts) {
		...
		if (!empty($opts['bind']) && is_array($opts['bind'])) {
	 	...
	 	//是否重命名上传的文件  by awen @2014-6-13
		if (!empty($opts['uploadRename'])) {
			$this->uploadRename = $opts['uploadRename'];
		}
	...
	protected function upload($args) {
		...
		foreach ($files['name'] as $i => $name) {
			...
			// modify by awen @2014-6-13 增加是否重命名
			if (($file = $volume->upload($fp, $target, $name, $tmpname,$this->uploadRename)) === false) {
