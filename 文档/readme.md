...
chinese,japanese,english）手续所有的多语言自动话部分依托于改key， 比如翻译，比如前端展示，比如域名，比如数据库，比如缓存。但是后台的显示部分可以自行定义支持的语言，不过需要和该配置中的语言的key相同，否则
当前语言信息在$GLOBALS['lang']中（也可以自己人工设定，但必须在加载view模板文件之前才能生效）。  
系统会自动去根目录下的**langs**文件夹中获取对应的模板页面的语言配置文件: *viewpathxxx_lang*（如果 /manage/m_index_english.json）,在view模板文件中可以直接调用变量名$lang来获取语言数据
